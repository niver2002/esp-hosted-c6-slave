...
static void process_rx_pkt(interface_buffer_handle_t *buf_handle)
{
	...
	if (buf_handle->if_type == ESP_STA_IF && station_connected) {
		/* Check if this is an aggregated packet: 
		 * Format: [DST_MAC(6)][SRC_MAC(6)][EtherType=0x9999(2)][magic=0xA6(1)][count(1)][len(2)][pkt]...
		 * Check: payload_len >= 18 (14B eth header + 2B aggr header + 2B first len)
		 *        EtherType at offset 12-13 == 0x9999
		 *        Magic byte at offset 14 == 0xA6
		 *        Count at offset 15: 2-24
		 */
		if (payload_len >= 18 && 
		    payload[12] == 0x99 && payload[13] == 0x99 &&
		    payload[14] == 0xA6 && 
		    payload[15] > 1 && payload[15] <= 24) {
			/* Aggregated packet detected */
			uint8_t count = payload[15];
			uint16_t offset = 16;  /* Skip Ethernet header (14B) + magic (1B) + count (1B) */
			int sent = 0;
